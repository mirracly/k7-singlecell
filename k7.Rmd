...
```{r}
# set a working dir in knitr
# (root.dir = "")
```

```{r}
files <- list.files()
files_filtered <- files[grep('_raw_feature_bc_matrix.h5', files)]
...
# The rest of the code is supposed to run pca, 
...
```{r}
comb_seurat <- FindNeighbors(comb_seurat, dims = 1:10)
comb_seurat <- FindClusters(comb_seurat, resolution = 0.5)
```

# visualize cell clusters.

```{r}
comb_seurat <- RunUMAP(comb_seurat, dims = 1:10)
```
=======
---
title: "Untitled"
...
DimPlot(comb_seurat, reduction = 'integrated.cca', group.by = 'singleR.labels')
```
...
>>>>>>> eb4ac00ba3c1d4d84361530bb5d22b551f3aad1e
